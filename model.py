...
class cycleGAN(object):
    def __init__(self, args):
        ...
        self.MSE = nn.MSELoss()
        self.L1 = nn.L1Loss()
        self.msssim = kornia.losses.SSIM(5, reduction='mean')
    ...
    def train(self, args):
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
                ...
                a_real_gray = gray((a_real + 1.) / 2)
                a_fake_gray = gray((a_fake + 1.) / 2)
                a_recon_gray = gray((a_recon + 1.) / 2)
                b_real_gray = gray((b_real + 1.) / 2)
                b_fake_gray = gray((b_fake + 1.) / 2)
                b_recon_gray = gray((b_recon + 1.) / 2)
                
#                 ba_ssim_loss = ((1. - self.msssim(a_real, b_fake)) + 
#                                 (1. - self.msssim(a_fake, b_recon))) * args.lamda * args.ssim_coef * 0.25
#                 ab_ssim_loss = ((1. - self.msssim(b_real, a_fake)) + 
#                                 (1. - self.msssim(b_fake, a_recon))) * args.lamda * args.ssim_coef * 0.25
