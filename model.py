    ...
    def train(self, args):
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
                ...
                a_real_gray = 2*(gray((a_real + 1.)/2)) - 1
                a_fake_gray = 2*gray((a_fake + 1.)/2) - 1
                a_recon_gray = 2*gray((a_recon + 1.)/2) - 1
                b_real_gray = 2*gray((b_real + 1.)/2) - 1
                b_fake_gray = 2*gray((b_fake + 1.)/2) - 1 
                b_recon_gray = 2*gray((b_recon + 1.)/2) - 1
                
                ba_ssim_loss = ((1. - self.msssim(a_real_gray, b_fake_gray)) + 
                                (1. - self.msssim(a_fake_gray, b_recon_gray))) * args.lamda * args.ssim_coef * 0.25
                ab_ssim_loss = ((1. - self.msssim(b_real_gray, a_fake_gray)) + 
                                (1. - self.msssim(b_fake_gray, a_recon_gray))) * args.lamda * args.ssim_coef * 0.25
